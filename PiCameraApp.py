...
try:
	...
	raise ImportError("You do not seem to have picamera2 installed")
...
class PiCameraApp ( Frame ):
	...
	def ToggleHFlip ( self ):
		...
		if self.HFlipState == True:
			preview_config["transform"] = libcamera.Transform(hflip=1)
		else:
			preview_config["transform"] = libcamera.Transform(hflip=0)
	...
	def ToggleVFlip ( self ):
		...
		self.camera.stop()
		preview_config = self.camera.preview_configuration()
		if self.VFlipState == True:
			preview_config["transform"] = libcamera.Transform(vflip=1)
		else:
			preview_config["transform"] = libcamera.Transform(vflip=0)
		self.camera.configure(preview_config)
		self.camera.start()
