...
class AboutDialog ( Dialog ):
	def BuildDialog ( self ):
		...
		n = ttk.Notebook(self.MainFrame,padding=(5,5,5,5))
...
class About ( BasicNotepage ):
	def BuildPage ( self ):
		...
		l = Label(self,text='Python version: %s' % platform.python_version())
		l.grid(row=6,column=0,sticky='NSEW')

		if NoRequire:
			# PiVer = "picamera2 library version unknown"
			# OpencvVer = "OpenCV library version unknown"
			PILVer = "Pillow (PIL) library version unknown"
			RPIVer = "GPIO library version unknown"
		else:
			# PiVer = "picamera2 library version %s" % require('picamera2')[0].version
			# OpencvVer = "OpenCV library version %s" % require('cv2')[0].version
		...
		# Label(self,text=OpencvVer).grid(row=7,column=0,sticky='NSEW')
		...
		if s:
			txt = 'Processor type: %s (%s)' % (platform.processor(), platform.machine())
		else:
			txt = 'Processor type: %s' % platform.machine()

		Label(self,text=txt).grid(row=10,column=0,sticky='NSEW')
		Label(self,text='Platform: %s' % platform.platform()).grid(row=11, column=0,sticky='NSEW')
...
class Credits ( BasicNotepage ):
	def BuildPage ( self ):
		...
		Label(f, text=string).grid(row=0,column=0,sticky='NSEW')
