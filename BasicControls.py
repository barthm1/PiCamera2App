...
class BasicControls ( BasicNotepage ):
	def BuildPage ( self ):
		...
		self.SetupLabelCombo(f,'Brightness:',0 ,0 ,-10, 10,
		# self.CameraBrightnessChanged, self.camera.brightness)
                self.CameraBrightnessChanged, 0.0)
		...
		self.SetupLabelCombo(f,'Contrast:',0,3,0, 320,
		# self.ContrastChanged, self.camera.contrast )
		self.ContrastChanged, 10)
	...
	def CameraBrightnessChanged ( self, newVal ):
		...
		aktval = float(newVal) / 10.0
		# print ("Newval ", aktval)
		self.camera.set_controls({"Brightness": aktval})
	...
	def ContrastChanged ( self, newVal ):
		...
		aktval = float(newVal) / 10.0
                # print ("Newval ", aktval)
		self.camera.set_controls({"Contrast": aktval})
	...
	def SaturationChanged ( self, newVal ):
		...
		aktval = float(newVal) / 10.0
                # print ("Newval ", aktval)
		self.camera.set_controls({"Saturation": aktval})
	...
	def SharpnessChanged ( self, newVal ):
		...
		aktval = float(newVal) / 10.0
                # print ("Newval ", aktval)
		self.camera.set_controls({"Sharpness": aktval})
