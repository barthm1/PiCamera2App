...
class Exposure ( BasicNotepage ):
	def BuildPage ( self ):
		...
		atuple = ("CentreWeighted", "Spot", "Matrix")
		l = list(atuple)
		l.sort()
		...
		btuple = ("Normal,", "Short", "Long", 'off', 'auto')
		exp = list(btuple)
		...
		dtuple = ("Incandescent", "Tungsten", "Fluorescent", "Indoor", "Daylight", "Cloudy", 'off', 'auto')
		modes = list(dtuple)
	...
	def CheckGains ( self ):
		...
		ag = metadata.get("AnalogueGain")
		dg = metadata.get("DigitalGain")
		...
		# self.MeasIso.config(text= 'Auto' if self.camera.iso == 0 else str(self.camera.iso) )
		self.MeasIso.config(text= 'Auto')
